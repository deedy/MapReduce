...
let create size =
  if size < 1 then failwith "Tpool create needs at least one thread" else
...
let add_work (f : unit -> unit) (tp : pool) =
  match tp with (nworkers, q, m, c) ->
    Mutex.lock m ;
    if !nworkers < 1
...
let rec done_wait (tp : pool) (n : int) =
  match tp with (nworkers, _, m, c) ->
    ...
    then Mutex.unlock m
      ...
      (Printf.printf "Tpool destroy, still waiting for %d threads.\n"  (n + !nworkers);
       flush stdout;
       Mutex.unlock m;
       Thread.delay 0.1;
       done_wait tp n)
