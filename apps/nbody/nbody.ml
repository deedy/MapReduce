...
(* Map, combine, reduce *)
let map_reduce (app_name : string) (mapper : string) (reducer : string)
    (kv_pairs : (string * string) list) (shared_data : string) =
...
(* Initializes an array using body ids *)
let get_graph_array_from_bodies (bodies : (string * body) list) : body array = 
  let foo acc (id,_) = if ((int_of_string id) > acc) then (int_of_string id)
                       else acc in
  let max_id = List.fold_left foo (-1) bodies in
...
(* Obtains body ids from bodies *)
let get_kv_pairs_from_bodies 
    (bodies: (string * body) list ) : (string * string) list = 
  List.rev (List.fold_left (fun acc (id,_) -> (id,"")::acc) [] bodies) in

(* Updates bodies based on results *)
let get_bodies_from_results (results : (string * string list) list) 
    (bodies_array: body array) : (string * body) list =
  let foo acc (id,accel_list) = let iden = int_of_string id in 
    ...
    (id,(mass,((posx+.velx+.(accx/.2.)),(posy+.vely+.(accy/.2.))),
      (velx+.accx,vely+.accy)))::acc in
  List.rev (List.fold_left foo [] results) in

(* Fills in array with bodies *)
let new_graph_array (bodies : (string * body) list) 
    (bodies_array: body array) : body array = 
  let foo acc (id,bod) = Array.set bodies_array (int_of_string id) bod; acc in
  let _ = List.fold_left foo 0 bodies in
  bodies_array in
...
(* Create a transcript of body positions for `steps` time steps *)
  ...
  let rec regenerate_positions 
      (bodies: (string * body) list) (steps: int) (transcript: string): string = 
    if (steps=0) then transcript else
    let results = 
      map_reduce "nbody" "mapper" "reducer" kv_pairs_m (marshal bodies_array) in
    let bodies = get_bodies_from_results results bodies_array in
    let _ = new_graph_array bodies bodies_array in
    let transcript = transcript^(string_of_bodies bodies) in
    let _ = get_kv_pairs_from_bodies bodies in 
